...
def lda(tf_idfs, topics): #does latent dirichlet analysis on tf-idf scores to find topics
...
def main():
	...
	topic_weights, doc_topics = lda(train_tf_idfs, num_topics)
	#do lda on tf-idfs and apply the results to get topics for dev set
	
	test_doc_topics = lda_transform(topic_weights, dev_tf_idfs, dev_reviews)
	...
	actual_polarities = [rating >= 7 for rating in dev_polarities]
